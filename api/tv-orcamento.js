...
async function createPipefyCard() { return { ok: false, error: 'Pipefy desconectado' }; }
  ...
  if (req.method === "POST" && action === "orc-status") {
    ...
    if (status === "aprovado" && ficha.pipefyId) {
      try {
        ...
        if (cd?.card?.current_phase?.id === "341638208") {
          await pipefyQuery(`mutation { moveCardToPhase(input: { card_id: "${ficha.pipefyId}", destination_phase_id: "341638194" }) { card { id } } }`);
        }
      } catch(e) { /* silencioso */ }
    }
...
async function gerarTextoOrcamento(desc, comentarios, nome) {
  ...
  if (!pol) {
    try {
      ...
      var sysMsg  = "Voce e Pedro da Reparo Eletro. Gere orcamento de TV seguindo EXATAMENTE este modelo:\n\nNOME bom dia, sou o Pedro da Reparo Eletro e vou passar seu orcamento:\n\nForam feito os testes, identificamos que sera necessario fazer a troca da [PECA] da TV, sera feito a reoperacao eletrica tambem. Este conserto completo fica em [VALOR] reais apenas. Aprovando ja iniciamos o conserto.\n\nCite SEMPRE o tamanho da TV em polegadas quando informado (ex: TV 50 polegadas). Substitua NOME pelo primeiro nome real (so o nome, sem Ola ou virgula antes). Mantenha [VALOR] literal. [PECA] = peca mais provavel (placa fonte, placa principal, placa T-CON, barramento, flat, memoria, solda). Responda so com o texto do orcamento.";
