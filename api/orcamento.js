...
async function pipefyQuery(query) {
  ...
  try {
    ...
    let json;
    try { json = JSON.parse(text); } catch(e) { throw new Error("INVALID_RESPONSE"); }
    if (json.errors) throw new Error(json.errors[0].message);
    return json.data;
  } finally { clearTimeout(timer); }
}

// Busca campos e fases do pipe dinamicamente
...
async function createPipefyCard({ phaseId, nome, telefone, aparelho, defeito, endereco }) {
  ...
  // Nome do contato = Nome + últimos 4 dígitos do telefone
  ...
  const telField  = findField(["telefone", "fone", "celular", "contato"]);
  const descField = findField(["descrição", "descricao", "descri", "problema", "servi", "observ"]);
  ...
  if (endField)  fieldsAttr.push(`{ field_id: "${endField.id}",   field_value: ${JSON.stringify(endereco)} }`);
