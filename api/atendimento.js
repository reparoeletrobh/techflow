...
export default async function handler(req,res){
  ...
  if(req.query.action==='metrics'){
    try{
      ...
      const todayStr=dStr();
      // Queries: pipe+ERP em paralelo com Redis, e fichas do mês via allCards separado
      ...
      const Q2='query{allCards(pipeId:"'+PID+'",filter:{created_at:{after:"'+monthISO+'"}}){totalCount}}';
      const[pd,cd,vd,hr,ficMesData]=await Promise.all([pf(Q1),dbG(CK),dbG(VK),dbG(HIST),pf(Q2).catch(()=>null)]);
      const fichasTotal=pd?.pipe?.cards_count||0;
      const fichasMes=ficMesData?.allCards?.totalCount??0;
      // Snapshots para hoje/semana
      ...
      const dp=prevSnap(todayStr),wp=prevSnap(dStr(weekBRT));
      ...
      // FIX: falta = todos os comprados este mes (eles ainda precisam ser cadastrados)
