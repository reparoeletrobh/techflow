...
module.exports = async function handler(req, res) {
  ...
  if (action === 'verificar-pagamento') {
    ...
    const finD = (await dbGet(FIN_KEY)) || { records:[] };
    const recD = (finD.records||[]).find(r =>
      (r.nome||'').toLowerCase().includes(q2) ||
      (r.nomeContato||'').toLowerCase().includes(q2) ||
      (r.tel||'').includes(q2)
    );
    if (!recD) return res.status(404).json({ ok:false, error:'Não encontrado: '+q2,
      amostra:(finD.records||[]).slice(0,5).map(r=>({id:r.id,nome:r.nome||r.nomeContato,fase:r.phaseId,paidAt:r.paidAt}))
    });
    const diagD = [];
    let statusMPD = null;
    const pidD = recD.paymentId || recD.mpPaymentId;
    if (pidD && MP_TOKEN) {
      try {
        const mpRD = await fetch('https://api.mercadopago.com/v1/payments/'+pidD,
          { headers:{ Authorization:'Bearer '+MP_TOKEN } });
        const mpD = await mpRD.json();
        statusMPD = { id:mpD.id, status:mpD.status, statusDetail:mpD.status_detail,
          valor:mpD.transaction_amount, payer:mpD.payer?.email||mpD.payer?.first_name||'—',
          dataAprovacao:mpD.date_approved, metodoPagamento:mpD.payment_method_id };
        if (mpD.status==='approved') diagD.push('✅ MP APROVADO — dinheiro entrou na conta');
        else if (mpD.status==='pending') diagD.push('⚠️ MP PENDENTE — pagamento não aprovado ainda');
        else if (mpD.status==='rejected') diagD.push('❌ MP REJEITADO — dinheiro NÃO entrou');
        else diagD.push('⚠️ Status MP: '+mpD.status+' / '+mpD.status_detail);
      } catch(eD){ diagD.push('❌ Erro ao consultar MP: '+eD.message); }
    } else if (!pidD) {
      diagD.push('🔴 SEM paymentId — ficha marcada como paga SEM confirmação do webhook MP');
      diagD.push('💡 Possível: PIX falso, link não pago, ou fase movida manualmente');
    }
    return res.status(200).json({ ok:true,
      ficha:{ id:recD.id, nome:recD.nome||recD.nomeContato, fase:recD.phaseId,
        valor:recD.valor, paidAt:recD.paidAt||null, paymentId:pidD||null },
      statusMP:statusMPD, diagnostico:diagD });
  }
