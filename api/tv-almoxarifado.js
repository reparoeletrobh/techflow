// ═══════════════════════════════════════════════════════════════════
// ALMOXARIFADO — TELEVISORES
// Mesma lógica do almoxarifado da linha branca, operando sobre os bancos
// do sistema de TV. Foi replicado em vez de compartilhado porque as duas
// frentes têm fases, peças e rotas próprias, e misturar os dados faria uma
// alterar o fluxo da outra sem aviso.
// ═══════════════════════════════════════════════════════════════════
...
export default async function handler(req, res) {
  ...
  if (action === 'sync' || action === 'load') {
    try {
      const [pipe, log] = await Promise.all([
        dbGet('tv_pipe'), dbGet('tv_logistica'),
      ]);
      ...
      try {
        const [fl, vnd, ckv, ceq] = await Promise.all([
          dbGet('tv_frenteloja'), dbGet('tv_vendas'),
          dbGet('tv_checkout_vendas'), dbGet('tv_compra_equip'),
        ]);
        const s = db.snapshot;
  ...
  if (action === 'coletados-sem-tarefa') {
    ...
    const [db, logA] = await Promise.all([dbGet(KEY), dbGet('tv_logistica')]);
  ...
  if (action === 'corrigir-tarefa') {
    ...
    const log = (await dbGet('tv_logistica')) || { fichas: [] };
  ...
  if (action === 'restaurar-comprados') {
    ...
    const ceq = (await dbGet('tv_compra_equip')) || { fichas: [], syncedIds: [] };
    ceq.fichas = ceq.fichas || [];
    const pp = (await dbGet('tv_pipe')) || { cards: [] };
    const arq = (await dbGet('tv_arquivo')) || { fichas: [] };
    ...
    await dbSet('tv_compra_equip', ceq);
    return res.status(200).json({ ok: true,
  ...
  if (action === 'procurar-backup') {
    const CANDIDATAS = [
      'tv_compra_equip_backup', 'tv_compra_equip_bak',
      'tv_compra_equip_old', 'tv_compra_equip_snapshot',
      'compra_equip_backup', 'almox_snapshot', 'tv_almoxarifado_snapshot',
      'tv_almoxarifado',
    ];
    const achados = [];
    ...
    const almox = await dbGet('tv_almoxarifado');
  ...
  if (action === 'quando-mudou') {
    const ceq = (await dbGet('tv_compra_equip')) || { fichas: [] };
  ...
  if (action === 'auditoria-compra') {
    const CHAVES = ['tv_compra_equip', 'compra_equipamentos', 'tv_equipamentos',
      'equipamentos_comprados', 'tv_compra_equip_bkp', 'tv_compra_equip_archive',
      'compra_equip', 'tv_arquivo', 'tv_pipe'];
    const achados = [];
  ...
  if (action === 'estado-compra-equip') {
    const ceq = (await dbGet('tv_compra_equip')) || null;
    if (!ceq) return res.status(200).json({ ok: false,
      error: '🚨 a chave tv_compra_equip NÃO EXISTE no banco' });
    const fichas = ceq.fichas || [];
  ...
  if (action === 'por-que-nao-entrou') {
    ...
    const [db, logA, logT, pipeA, pipeT] = await Promise.all([
      dbGet(KEY), dbGet('tv_logistica'), dbGet('tv_logistica'),
      dbGet('tv_pipe'), dbGet('tv_pipe'),
    ]);
  ...
  if (req.method === 'POST' && action === 'concluir') {
    ...
    if (t.tipo === 'mover' && t.destino === 'aprovados') {
      try {
        const bdb = await dbGet('tv_board');
    ...
    if (t.tipo === 'avaliar-compra') {
      ...
      try {
        const cdb = await dbGet('tv_compra_equip');
        ...
        if (cf) {
          ...
          await dbSet('tv_compra_equip', cdb);
        }
      } catch (e) {}
  ...
  if (req.method === 'POST' && action === 'rota-separar') {
    ...
    const rdb1 = (await dbGet('tv_almox_rotas')) || { rotas: [] };
    ...
    await dbSet('tv_almox_rotas', rdb1);
  ...
  if (req.method === 'POST' && action === 'rota-negar') {
    ...
    if (!motivo) return res.status(400).json({ ok: false, error: 'motivo obrigatório' });
    const rdb2 = (await dbGet('tv_almox_rotas')) || { rotas: [] };
    ...
    await dbSet('tv_almox_rotas', rdb2);
    return res.status(200).json({ ok: true });
  }
  ...
  if (req.method === 'POST' && action === 'rota-saida') {
    ...
    const rdb3 = (await dbGet('tv_almox_rotas')) || { rotas: [] };
    ...
    await dbSet('tv_almox_rotas', rdb3);
  ...
  if (action === 'arquivo') {
    ...
    const rdbA = (await dbGet('tv_almox_rotas')) || { rotas: db.rotas || [] };
  ...
  if (action === 'rota-foto') {
    ...
    if (!f || !f.b64) {
      ...
      const rdbF = (await dbGet('tv_almox_rotas')) || { rotas: [] };
  ...
  if (action === 'motoristas') {
    const [rdbM, lgT] = await Promise.all([dbGet('tv_almox_rotas'), dbGet('tv_logistica')]);
  ...
  if (action === 'buscar-tudo') {
    ...
    const [rdb, arqA, arqT, ppA, ppT, lgA, lgT, movs] = await Promise.all([
      dbGet('tv_almox_rotas'), dbGet('tv_arquivo'), dbGet('tv_arquivo'),
      dbGet('tv_pipe'), dbGet('tv_pipe'),
      dbGet('tv_logistica'), dbGet('tv_logistica'),
      dbGet('tv_movimentacoes'),
    ]);
  ...
  if (action === 'badge') {
    ...
    const rdbB = (await dbGet('tv_almox_rotas')) || { rotas: db.rotas || [] };
  ...
  if (action === 'reconciliar-dia') {
    ...
    const [pdb, pdbTv] = await Promise.all([dbGet('tv_pipe'), dbGet('tv_pipe')]);
    const faltam = [];
  ...
  if (action === 'faxina') {
    ...
    try {
      const [pdb, ldb] = await Promise.all([dbGet('tv_pipe'), dbGet('tv_logistica')]);
  ...
  if (req.method === 'POST' && action === 'ml-chegou') {
    ...
    const KEYP = sistema === 'tv' ? 'tv_compras_pecas' : 'tv_compras_pecas';
    ...
    try {
      const pd = (await dbGet('tv_pecas_disponiveis')) || { itens: [] };
      ...
      await dbSet('tv_pecas_disponiveis', pd);
    } catch (e) {}
