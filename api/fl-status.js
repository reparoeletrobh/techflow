// api/fl-status.js — Status FL x Board em tempo real
  ...
  try {
    ...
    const resultado = fichas
      ...
      .map(f => {
        ...
        let status;
        if (boardPhase === 'sem_card') status = 'SEM_CARD';
        else if (expectedFL === f.phase) status = 'OK';
        else status = 'ERRO';

        const hist = f.history || [];
        const prodEntry = hist.filter(h => h.phase === 'producao').slice(-1)[0];
        const data = (prodEntry?.ts || f.movedAt || f.criadoEm || '').slice(0,10);
      ...
      .sort((a,b) => {
        ...
        return (order[a.split(',')[6]]||3) - (order[b.split(',')[6]]||3);
      });

    const csv = ['ID,Nome,Equipamento,FL_Phase,Board_Phase,Esperado_FL,Status,Data']
