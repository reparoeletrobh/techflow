...
async function renderFiltrado() {
  ...
  // Busca valor real dos ERPs do período diretamente no Pipefy
  try {
    var vr = await fetch('/api/metricas?action=valor-erp-periodo&de=' + de + '&ate=' + ate).then(function(r){return r.json();});
    if (vr.ok && vr.valor > 0) m.valorErp = vr.valor;
  } catch(e) {}
...
function renderStrip(m) {
  ...
  // ERP count: metaLog do período (ou ao vivo sem filtro)
  var erpCount  = filtroAtivo ? (m.erpCount || 0) : e.count;
  // Valor ERP:
  //   sem filtro → ao vivo Pipefy
  //   com filtro e m.valorErp > 0 (lançado manual) → usa lançado
  //   com filtro e m.valorErp = 0 → usa ao vivo (melhor estimativa)
  var erpValorManual = filtroAtivo ? (m.valorErp || 0) : 0;
  var erpValor  = !filtroAtivo ? e.valor : (erpValorManual > 0 ? erpValorManual : e.valor);
  // Ticket Médio: valorErp / erpCount
  var ticketLive = erpCount > 0 ? (erpValor / erpCount).toFixed(2) : null;
  // Todos os 3 cards do topo sempre usam dados ao vivo do Pipefy
  var erpBadge  = '<div class="live-badge">AO VIVO</div>';
  var valBadge  = '<div class="live-badge">AO VIVO</div>';
  var roasBadge = '<div class="live-badge" style="background:rgba(6,182,212,.12);color:var(--cyan);border-color:rgba(6,182,212,.2);">AO VIVO</div>';
  var roasVal = m.investimento > 0 ? (erpValor / m.investimento).toFixed(2) + 'x' : '—';
  var erpLabel  = filtroAtivo ? 'ERPs no Período' : 'ERPs em Aberto';
  var valLabel  = 'Valor Total ERP';
  var erpSub    = filtroAtivo ? 'entraram em ERP no período' : 'cards na fase ERP';
  var valSub    = filtroAtivo ? 'soma dos contratos do período' : 'soma dos contratos ao vivo';
  var rowLabel  = '📡 ERP ao vivo — Pipefy';
  ...
  document.getElementById('live-strip').innerHTML =
    ...
    '<div class="strip-row periodo">' +
      ...
      '<div class="live-card purple">' +
        ...
        '<div class="lc-value purple">' + BRL(ticketLive) + '</div>' +
        '<div class="lc-sub">valor ERP / ERPs no período</div>' +
      metaBar(ticketLive, METAS.ticketMedio) +
      '</div>' +
...
async function salvarDia() {
  ...
  var r = await fetch('/api/metricas?action=salvar-dia', {
    method: 'POST',
...
async function deletarDia(dataVal) {
  ...
  var r = await fetch('/api/metricas?action=deletar-dia', {
...
(function init() {
  ...
  // Filtro de período: padrão = hoje — seta ANTES do load()
  ...
  // Carrega dados — filtroAtivo já está setado, load() usará o filtro correto
  load();
