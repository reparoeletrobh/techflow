...
export default async function handler(req, res) {
  ...
  if (req.method==='POST' && action==='editar-pipe') {
    ...
    if(card.valor>0&&!card.orcamentoEm){card.orcamentoEm=new Date().toISOString();_funil.registrar('orcamento',{telefone:card.telefone,nome:card.nomeContato||card.nome,valor:card.valor,equipamento:card.equipamento||card.descricao,frente:'tv',canal:'online',ref:card.id}).catch(()=>{});}
