...
module.exports = async function handler(req, res) {
  ...
  if (action === 'arquivar-automatico') {
    ...
    const db = await dbGet(LOG_KEY) || defaultDB();
    const arq = (await dbGet('reparoeletro_logistica_arquivo')) || { fichas: [] };
    ...
    await dbSet('reparoeletro_logistica_arquivo', arq);
    await dbSet(LOG_KEY, db);
  ...
  if (action === 'buscar-arquivo') {
    ...
    const arqB = (await dbGet('reparoeletro_logistica_arquivo')) || { fichas: [] };
  ...
  if (req.method === 'POST' && action === 'enviar-prospeccao') {
    ...
    try {
      ...
      cont.dias[hojeR].adm = (cont.dias[hojeR].adm || 0) + 1;
    ...
    try {
      const evt = JSON.stringify({ ts: now, tipo: 'entrar_contato', de: null, sis: 'adm', origem: 'remarcar', id: nova.id, nome: nova.nome });
